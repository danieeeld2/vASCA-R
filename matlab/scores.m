function figH = scores(model,varargin)
    ...
    % Check if ObsTest was provided. If not, try to use model.test
    if isempty(test) && isfield(model, 'test')
        test = model.test;
        L = size(test, 1);
    end
    ...
    elseif strcmp(plottype,'Scatter')
        if ~isempty(Tt) % Check if Tt is not empty before plotting
