function vascao = vasca(parglmoVS,siglev)
    ...
    % Check if parglmoVS is a structure because it is a required in octave
    if iscell(vascao.factors)
        factorsTemp = vascao.factors;
        vascao.factors = struct([]);
        for i = 1:length(factorsTemp)
            vascao.factors(i) = factorsTemp{i};
        end
    end
    
    if iscell(vascao.interactions)
        interactionsTemp = vascao.interactions;
        vascao.interactions = struct([]);
        for i = 1:length(interactionsTemp)
            vascao.interactions(i) = interactionsTemp{i};
        end
    end
